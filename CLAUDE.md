...
## WhatsApp Number
ALWAYS check `.Site.Params.whatsapp` in hugo.toml before any deploy.
Must NOT be `"+57300000000"` (placeholder). Current real number: `+573163946401`.

## SEO Requirements (every page must have)
- Unique `<title>` tag
- Meta description (from content front matter or site default)
- Canonical URL (auto-generated by baseof.html)
- OG tags: og:title, og:description, og:type, og:url (in baseof.html)
- hreflang for bilingual pages (in baseof.html)

## Schema Markup
- Events page: JSON-LD `ItemList` of `Event` objects for all upcoming events
- All pages: JSON-LD `WebSite` schema (in baseof.html)

## Image Requirements
- Always use `<picture>` with WebP source + JPG fallback
- Always include `width` and `height` attributes on `<img>` tags
- `loading="lazy"` on all images except the first/hero
- First image on list pages: `fetchpriority="high"` (passed via `isFirst` flag in work/section.html)
- Pass `isFirst` to place-card.html using: `(merge $place (dict "isFirst" (eq $i 0)))`

## Monetization hooks (future)
When adding monetization, use these patterns only:
- Affiliated booking links with `rel="sponsored"` and visible disclosure
- "Featured" label on paid listings (never disguised as editorial)
- WhatsApp concierge: link from every page, never a form
- Casa Gaviota: one page, always disclosed, never ranked above other listings
